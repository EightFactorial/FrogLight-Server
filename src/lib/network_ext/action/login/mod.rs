...
pub struct ConnectionLoginPlugin<V: Version>(PhantomData<V>);

impl<V: Version + LoginProfileTrait> Plugin for ConnectionLoginPlugin<V>
where
...
impl<V: Version + LoginProfileTrait> ConnectionLoginPlugin<V>
where
    Clientbound: NetworkDirection<V, Login>,
    Login: State<V>,
{
    /// A system that sends clients [`GameProfile`]s.
    pub fn send_connection_profiles(
